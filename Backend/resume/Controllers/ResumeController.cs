...
namespace resume.Controllers
{
    ...
    public class ResumeController : ControllerBase
    {
        ...
        public ResumeController(CompanyService companyService, ResumeService resumeService, ApplicantService applicantService, IServiceScopeFactory scopeFactory)
        {
            _companyService = companyService;
            _resumeService = resumeService;
            _applicantService = applicantService;
            _scopeFactory = scopeFactory; // 初始化 IServiceProvider
        }
        /// <summary>
        ...
        public FirstAddResumeModelClass UploadResume(IFormFile file, int jobId, int UserId)
        {
            ...
            string newFileName_1 = hashedFileName + ".jpg";
            string fileUrlWithoutFileName_1 = @$"{DateTime.Now.Year}\{DateTime.Now.Month}\{"image"}";
            string filePath_1 = Path.Combine(Directory.GetCurrentDirectory(), $@"{staticFileRoot}\{fileUrlWithoutFileName_1}", newFileName_1);
        ...
        private static EmailResumeProcessor _emailProcessor;


        [HttpPost("bind_mail_count")]
        public ActionResult<string> BindMailAndCount(string emailAddress, string emailPassword)
        {
            //emailAddress = "2141377591@qq.com";
            //emailPassword = "ljfaepslzrhzbeii";
            if (string.IsNullOrEmpty(emailAddress) || string.IsNullOrEmpty(emailPassword) )
            {
                return BadRequest("需要提供邮箱地址、密码和存储路径.");
            }

            // 创建 EmailResumeProcessor 实例并绑定邮箱
            _emailProcessor = new EmailResumeProcessor(_scopeFactory);
            _emailProcessor.StartListening(emailAddress, emailPassword);

            // 返回提示信息
            return Ok("邮箱绑定成功并开始监听新邮件。");
        }

        private void ResumeReceivedCallback(string resumeFilePath)
        {
            // 在这里处理接收到的简历文件，解析简历，生成图片路径，存储申请人信息和简历路径等操作
            try
            {
                ...
                string fileName = Path.GetFileNameWithoutExtension(resumeFilePath); // 获取不带扩展名的文件名
                Console.WriteLine($"文件名：{fileName}");

                // 生成图片路径
                Console.WriteLine("生成图片路径...");
                string staticFileRoot = "Resumes";
                string newFileName_1 = fileName + ".jpg"; // 使用相同的文件名，但扩展名为.jpg
                string fileUrlWithoutFileName_1 = @$"{DateTime.Now.Year}\{DateTime.Now.Month}\{"image"}";
                string filePath_1 = Path.Combine(Directory.GetCurrentDirectory(), $@"{staticFileRoot}\{fileUrlWithoutFileName_1}", newFileName_1);
                ...
                int resumeID = _resumeService.AddResumePath(resumeFilePath, filePath_1, storedApplicantId, 1, 1);
