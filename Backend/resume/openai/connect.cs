        ...
        */
        ...
        private static void getFile(string directoryPath, string fileName, string extension, int n)
        {
            try
            {
                ...
                start.FileName = @"E:\software\Anaconda3\install\envs\resume\python.exe";
                start.Arguments = string.Format("{0} \"{1}\" {2} \" {3} \" {4} \" {5}", @"E:\study\software_engineering\term_project\old_code\resume\resume\resume\openai\fileRelate.py", directoryPath, fileName, extension, n, key);
                ...
                using (Process process = Process.Start(start))
                {
                    ...
                    // process.WaitForExit(); change this line with:
                    bool exited = process.WaitForExit(200000);  // Wait for 10 minutes

                    if (!exited)
                    {
                        Console.WriteLine("Python script execution timed out");
                    }
        ...
        private static async Task<Dictionary<string, object>> Run1(string filepath)
        {
            ...
            string key = "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";
            ...
            string schoolRanksFilePath = @"E:\study\software_engineering\term_project\old_code\resume\resume\resume\openai\school.json";
        ...
        private static async Task<Dictionary<string, object>> Run2(string filepath)
        {
            ...
            string key = "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";
        ...
        private static Task<string> RunPythonScript(string filepath, string fileName, string extension, int n, int maxTries, string key)
        {
            ...
            try
            {
                ProcessStartInfo start = new ProcessStartInfo();
                start.FileName = @"E:\software\Anaconda3\install\envs\resume\python.exe";
                start.Arguments = string.Format("{0} \"{1}\" {2} \" {3} \" {4} \" {5}", @"E:\study\software_engineering\term_project\old_code\resume\resume\resume\openai\chat.py", filepath, fileName, extension, n, key);
